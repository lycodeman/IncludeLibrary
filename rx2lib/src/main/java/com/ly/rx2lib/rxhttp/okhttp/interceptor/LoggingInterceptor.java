package com.ly.rx2lib.rxhttp.okhttp.interceptor;


import com.google.gson.Gson;
import com.ly.rx2lib.modle.response.ErroResponse;
import com.ly.rx2lib.rxhttp.result.HttpResponseException;
...
public class LoggingInterceptor implements Interceptor {
    ...
    private void printlnRequestInfo(Request request) {
        ...
        LUtils.d(stringBuilder.toString());
    }
            ...
            if (mediaType != null) {
                ...
                if (isTextMediaType(mediaType)) {
                    try {
                        ...
                        /*---------------------------------------此处需定制--------------------------------------------*/
                        if (str.contains("errorCode")&&str.contains("message")){
                            ErroResponse erroResponse=
                                    new Gson().fromJson(str, ErroResponse.class);
                            throw new HttpResponseException(erroResponse.getErrorCode().getMessage(),erroResponse.getErrorCode().getCode());
                        }
                    } catch (IOException e) {
